...
SELECT d.DepartmentID, d.Name, (m.FirstName + ' ' + m.LastName) AS  ManagerFullName
...
/**
  * 21. Write a SQL query to find all employees, along with their manager 
...
SELECT m.FirstName + ' ' + m.LastName AS ManagerFullName,
...
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, d.Name as DepartmentName, e.HireDate
	FROM Employees e
		JOIN Departments d 
			...
			WHERE (d.Name = 'Sales' OR d.Name = 'Finance') AND (YEAR(e.HireDate) > 1995 AND YEAR(e.HireDate) < 2000)